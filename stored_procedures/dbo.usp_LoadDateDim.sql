...
1.1       03/17/2021   JCRomero     1. Enhance with @v_num_days for better precision
...
SELECT CONVERT(VARCHAR, (@v_first_date + n.n), 112) AS date_id
     ...
     , CASE WHEN DATEPART( dd, (@v_first_date + n.n )) 
	   IN (30,31) THEN 1
	   ELSE 0 
     ...
     , CASE WHEN DATEPART( dy, (@v_first_date + n.n )) 
	   IN (31,59,90,120,151,181,212,243,273,304,334,365) THEN 1
	   ELSE 0
	   END  AS last_day_in_month_flag

     , DATEPART( qq, (@v_first_date + n.n )) AS quarter_number
     , CASE WHEN DATEPART(quarter , (@v_first_date + n.n )) = DATEPART(quarter, 1) THEN 'First'
	        WHEN DATEPART(quarter , (@v_first_date + n.n )) = DATEPART(quarter, 2) THEN 'Second'
			WHEN DATEPART(quarter , (@v_first_date + n.n )) = DATEPART(quarter, 3) THEN 'Third'
			WHEN DATEPART(quarter , (@v_first_date + n.n )) = DATEPART(quarter, 4) THEN 'Fourth'
       END  AS quarter_name

	 , CASE WHEN DATEPART(quarter , (@v_first_date + n.n )) = DATEPART(quarter, 1) THEN 'Q1'
	        WHEN DATEPART(quarter , (@v_first_date + n.n )) = DATEPART(quarter, 2) THEN 'Q2'
			WHEN DATEPART(quarter , (@v_first_date + n.n )) = DATEPART(quarter, 3) THEN 'Q3'
			WHEN DATEPART(quarter , (@v_first_date + n.n )) = DATEPART(quarter, 4) THEN 'Q4'
       END  AS quarter_code
...
SELECT 99991231 AS date_id
     ...
     , 12 AS month_number
     , 0 AS month_name
     , 0 AS month_abbreviation
     , 12 AS month_last_day_number
